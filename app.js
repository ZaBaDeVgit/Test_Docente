(function() {
  ...
  function loadData(){fetch('questions.json?v='+Date.now()).then(r=>{if(!r.ok)throw new Error('no data');return r.json()}).then(d=>{Q=d;S.view='home';render()}).catch(()=>{el.innerHTML='<div class="loader"><div class="loader-inner"><p style="color:#fda4af">Error cargando preguntas</p><button class="btn btn-primary" onclick="App.startApp()" style="margin-top:12px">Reintentar</button></div></div>'})}
